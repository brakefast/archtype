...
<c:choose>
    <c:when test="${disabled=='true'}">
        <form:checkboxes path="${id}"  items="${items}" itemLabel="${label}" itemValue="${value}" htmlEscape="false"  class="i-checks disabled"/>
    </c:when>
    <c:otherwise>
        <form:checkboxes path="${id}" items="${items}" itemLabel="${label}" itemValue="${value}" htmlEscape="false" class="i-checks "/>
    </c:otherwise>
</c:choose>
