var FormUtils = {
    ...
    convertSimpleForm2ObjectBySelector2: function (formSelector, obj) {
        obj = obj || {};
        var fields = $(formSelector).find("input:text:not([name$='_fd_name']),input:hidden,input:password,select,textarea");
        ...
        fields.each(function (index, domObj) {
            ...
            if (item.attr("name")) {
                if (item.attr("type") && item.attr("type").toLowerCase() == "radio") {
                    if (item.prop("checked") && (item.prop("checked") === true || item.prop("checked") == "true" || item.prop("checked") == "checked")) {
                    ...
                    if (item.attr("formatRules") && item.attr("formatRules").length > 0 && $.parseJSON(item.attr("formatRules")).checkedValueMap) {
                        if (obj[itemName] && obj[itemName] != "") {
                            obj[itemName] += "," + item.val();
                        } else {
                            obj[itemName] = item.val();
                        }
                    } else {
                        if (item.prop("checked") != undefined) {
                            if (item.prop("checked") || item.prop("checked") == "true" || item.prop("checked") == "checked") {
                                if (obj[itemName] && obj[itemName] != "") {
                                    obj[itemName] += "," + item.val();
                                } else {
                                    obj[itemName] = item.val();
            ...
            if (item.attr("name")) {
                if (item.attr("type") && item.attr("type").toLowerCase() == "radio") {
                    ...
                    if (item.attr("formatRules") && item.attr("formatRules").length > 0 && $.parseJSON(item.attr("formatRules")).checkedValueMap) {
                        ...
                        var itemName = item.attr("name");
                        var objValue = obj[itemName];
                        if (objValue && objValue != ""){
                            var vals = objValue.split(",");
                            if (vals.length > 1){
                                var result = $.inArray(item.val(), vals);
                                if (result > -1){
                    ...
                    if (FormUtils._containStr(domObj, "datePicker") || item.hasClass("datepicker")) {
...
//							|| item.attr("name").indexOf('_code_name_config') != -1){// Select Text
    ...
    clearForm4Selector: function (selector) {
        ...
        var spinnerFields = $(".spinner-class", selector)//数字控件清空
        ...
        //数字控件清空
        spinnerFields.each(function (index, domObj) {
            var item = $(domObj);
            item.attr("value", "");
        });
    ...
    disableForm: function (formId, isDisabled) {
        ...
        //数字控件禁用
        if (isDisabled) {//禁用
            $("form[id='" + formId + "'] .spinner-btn-group").hide(); //数字控件隐藏控制
        } else {
            $("form[id='" + formId + "'] .spinner-btn-group").show();
        }


        $(".edit-page :button").attr("disabled", isDisabled);
        $(".edit-page-panel :button").attr("disabled", isDisabled);
        $(".attachment").attr("disabled", false);
        $('#showBtn').removeAttr('disabled');//查看流程不禁用
        $("#printBtn").removeAttr("disabled");//打印按钮不禁用
        $("#print").removeAttr("disabled");
        $("#signetUser").removeAttr("disabled");//印章使用按钮不禁用
        $('#showBtn1').removeAttr('disabled');//查看流程不禁用
        $('#unapprove').removeAttr('disabled');//取消核准不禁用
    },
    // 放开按钮控制、禁用单据
    disableFormOnly: function (formId) {
        $(".edit-page :button").attr("disabled", false);

        $("form[id='" + formId + "'] .layer-date").addClass("readonly");
        $("form[id='" + formId + "'] :text").attr("disabled", true);
        $("form[id='" + formId + "'] textarea").attr("disabled", true);
        ...
        $("form[id='" + formId + "'] :button").attr("disabled", true);
        $("form[id='" + formId + "'] .layer-date").attr("disabled", true);
    },
